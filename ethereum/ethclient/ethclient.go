...
// TransactionByHash returns the transaction with the given hash.
func (ec *Client) TransactionByHashNotify(ctx context.Context, hash common.Hash) (height *string, isPending bool, err error) {
	var json *rpcTransaction1
	err = ec.c.CallContext(ctx, &json, "eth_getTransactionByHash", hash)
	if err != nil {
		return nil, false, err
	} else if json == nil {
	...
	return json.txExtraInfo.BlockNumber, true, nil
}
